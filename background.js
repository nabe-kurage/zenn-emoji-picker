...
async function callAPI(apiType, apiKey, text) {
  const prompt = `以下のZenn記事の内容を分析し、記事のテーマ・雰囲気・トピックに合った最適な絵文字を提案してください。

※毎回同じ絵文字ではなく、記事の内容に合わせて柔軟に選んでください。
※「🌸」「🦋」「✨」は例として提示しているだけで、毎回使う必要はありません。
※出力する絵文字は、記事の内容を象徴するものを選んでください。

記事内容:
${text}

以下のJSON形式で出力してください（絵文字は実際の内容に合ったものを選んでください）:
{
  "main": { "emoji": "ここに最も適切な絵文字", "reason": "記事のメインテーマに合った理由" },
  "sub": [
    { "emoji": "サブ提案1の絵文字", "reason": "理由" },
    { "emoji": "サブ提案2の絵文字", "reason": "理由" }
  ]
}`;
